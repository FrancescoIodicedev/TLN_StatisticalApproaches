...
# return a map with
#  [ { term_A, { termA1: Freq_termA1 , termA2: Freq_termA2  .. termA3: Freq_termA3 },
#    { term_B, { ..} ... ]
#   @ term_A to describe
#   @ termA1 , termA2 ..  : tutti i termini presenti nelle definzioni
#   @ Freq_termA1 .. valore che indica la frequenza del termine termA1
...
def clean_data(matrix):
    ...
    for col in range(len(matrix)):
        ...
        term_freq = get_frequency_of_words(all_terms)
        map_result[concept[1]] = term_freq
    return map_result
    ...
    for defs in matrix:
        ...
        for i, gloss in enumerate(defs):
            if i == 0:
                ...
                terms_freq.append(order_on_frequency(stemming_term, stats[keyname]))
            freq_on_def_terms[keyname] = terms_freq
    return freq_on_def_terms


def get_frequency_of_words(terms):
    result = []
    for w in set(terms):
        val = terms.count(w)
        # if val > 1:
...
def order_on_frequency(terms, sts_key_def):
    ordered_terms = []
    index = 0
...
if __name__ == '__main__':
    ...
    for key in topic_single_def:
        ...
        media_similarity[' '.join(key_val)] = np.mean(similarity)
    ...
    for specificity in dimension[1]:
        ...
        for concreteness in dimension[0]:
            ...
            sim_value.append(round(media_similarity[key], decimal_places))
